...
export function OffresClient() {
  const [offres] = useState<Offre[]>(DEMO_OFFRES);
              ...
              {filtered.map(offre => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className={`inline-block text-xs font-medium px-2 py-0.5 rounded-full ${STATUT_LABELS[offre.statut]?.cls}`}>
                      {STATUT_LABELS[offre.statut]?.label}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-center">
                    {offre.assigne_a ?? (
                  ...
                  {hoveredId === offre.id && (
                    <td className="absolute right-0 top-full z-30 mt-1 mr-4" colSpan={9}>
                      <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-3 text-xs w-64">
                        ...
                        <p className="text-gray-600">Catégories : {[...new Set(offre.produits.map(p => p.categorie))].join(', ')}</p>
